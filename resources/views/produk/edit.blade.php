...
@section('content')
<div class="mx-5 mt-5">
    <h3><a href="{{ route('produks.index') }}" class="text-decoration-none text-secondary">Daftar Produk</a> > Edit produk</h3>
    <div class="card" style="width: 100%;">
        <form action="{{ route('produks.update', $data->id) }}" method="POST" class="p-5" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="row mb-3">
                <div class="col-4">
                    ...
                    <select name="category" class="form-select" id="">
                        <option value="{{ $data->category_id }}">{{ \App\Models\Category_produk::find($data->category_id)->nama }}</option>
            ...
            <div class="row mb-3">
                <div class="col">
                    ...
                    <input type="number" name="harga_beli" value="{{ $data->harga_beli }}" id="harga_beli" class="form-control">
                    @error('harga_beli')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="col">
                    ...
                    <input type="number" name="harga_jual" value="{{ $data->harga_jual }}" id="harga_jual" class="form-control">
                    @error('harga_jual')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="col">
                    ...
                    <div class="custom-file-upload text-center border p-3 rounded">
                        <label for="image-upload" class="d-block">
                            <img src="{{ asset('assets/img/produk/'.$data->image) }}') }}" alt="Upload Icon" class="img-thumbnail mb-2" style="width: 100px; height: 100px;">
